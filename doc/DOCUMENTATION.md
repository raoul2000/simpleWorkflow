...
protected/models/Post.php beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); ?> // replace... array('status', 'in', 'range'=>array(1,2,3)), // ...with >array('status', 'SWValidator'), endWidget(); ?>
...
protected/views/post/_form.php (line 37)	beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); ?> // replace... echo $form->dropDownList($model,'status',Lookup::items('PostStatus')); // ...with echo $form->dropDownList($model,'status',SWHelper::nextStatuslistData($model)); endWidget(); ?>
...
protected/views/post/admin.php (line 19-20)	beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); ?> // replace ... 'value'=>'Lookup::item("PostStatus",$data->status)', 'filter'=>Lookup::items('PostStatus'), // ... with 'value'=>'CHtml::encode($data->status)', 'filter'=> SWHelper::allStatuslistData($model), endWidget(); ?>
...
beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); echo ' return array( 'initial' => 'draft', 'node' => array( array( 'id'=>'draft', 'transition'=>array( 'correction' => 'Yii::app()->mailManager->sendCorrectionMail()' ), // etc ... ) ) ?> endWidget(); ?>
...
beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); echo ' return array( 'initial' => 'draft', 'node' => array( // .... array( 'id'=>'ready', 'constraint'=>'Yii::app()->user->checkAccess('validateCorrection')', 'transition' =>'draft,correction,published' ), // ... ) ) ?> endWidget(); ?>
...
beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); echo ' public function rules() { return array( array('status','SWValidator'), // other validators ... ); } ?> endWidget(); ?>

Second, let's see how to validate model attributes (other that status) depending on workflow transitions :
beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); echo ' public function rules() { return array( array('status', 'SWValidator','enableSwValidation'=>true), array('category','required','on'=>'sw:Draft_Correction'), array('tags', 'required','on'=>'sw:Correction_Ready'), array('priority','required','on'=>'sw:Ready_Published'), // other validators ... ); } ?> endWidget(); ?>
...
beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); echo ' public function rules() { return array( array('status', 'SWValidator','enableSwValidation'=>true), array('category','required','on'=>array('sw:Correction_Ready','sw:Correction_Draft'), // other validators ... ); } ?> endWidget(); ?>
...
beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); echo ' public function rules() { return array( array('status', 'SWValidator','enableSwValidation'=>true,'match'=>true), // all models leaving the 'Correction' status, must have a category array('category','required','on'=>'sw:/Correction_.*/'), // all models that enter the 'Ready' status must have tags array('tags','required','on'=>'sw:/.*_Ready/'), // other validators ... ); } ?> endWidget(); ?>
Events
...
beginWidget('system.web.widgets.CTextHighlighter',array( 'language'=> 'PHP', 'showLineNumbers'=> false, )); ?> $m=MyModel::model()->findByPk('1'); if( $m->swHasStatus() ){ echo 'status : '.$m->swGetStatus()->toString(); }else { $m->swInsertToWorkflow(); $m->save(); } endWidget(); ?>

The API documentation is available here. In particular you may be intrested in the SWActiveRecordBehavior class which implements all the simpleWorkflow logic.
References
